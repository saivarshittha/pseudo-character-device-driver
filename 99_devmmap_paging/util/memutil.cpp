...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    int option_index = 0;
    static struct option long_options[] = {{"help", 1, 0, 'h'},
                                           {"ptype", 1, 0, 'p'},
                                           {"message", 1, 0, 'm'},
                                           {"operation", 1, 0, 'o'},
                                           {0, 0, 0, 0}};
  ...
  for (int i = 0; i < op_cnt; i++) {
    ...
    case OP_MAPREAD: {
      off_t off = 0;
      size_t len = MYDEV_LEN; // 1MB size
      char *device_memory;
      // memory map the devicemem's kernel buffer into user-space segment.
      // TODO
      device_memory =
          (char *)mmap(NULL, len, PROT_READ, mmap_flags, dev_fd, off);
      if (device_memory == MAP_FAILED)
        exit(EXIT_FAILURE);
      // Compare the data read from devicemem with msg
      // DONOT define an array of MYDEV_LEN. Seriously! Dont need array of
      // MYDEV_LEN size.
      // TODO. Hint use loop & modulus operator on msg to compare the string
      // with entire device_mem
      unsigned long i = 0;
      if (msg_len != 0) {
        while (i < len) {
          if (*(device_memory + i) != msg[i % msg_len])
            exit(EXIT_FAILURE);
          i++;
        }
      } else {
        while (i < len) { // not an error case
          if (*(device_memory + i) != '\0')
            continue; // just we are accessing each element of device mem
          i++;
        }
      }
      // unmap the devicemem's kernel buffer.
      // TODO
      munmap(device_memory, len);
      break;
    }
    ...
    case OP_MAPWRITE: {
      ...
      // memory map the devicemem's kernel buffer into user-space segment.
      // TODO
      device_memory = (char *)mmap(NULL, len, PROT_READ | PROT_WRITE,
      ...
      // DONOT define an array of MYDEV_LEN. Seriously! Dont need array of
      // MYDEV_LEN size.
      // TODO. Hint use loop & modulus operator on msg to copy the string to
      // entire device_mem
      unsigned long i = 0;
      if (msg_len != 0) {
        while (i < MYDEV_LEN) { // we keep on adding message content to the
                                // device file until 1MB is filled
          *(device_memory + i) = msg[i % msg_len];
          i++;
        }
      }
      // unmap the devicemem's kernel buffer.
...
void print_help(const char *name, int exit_value) {
  ...
  cerr << "--message <message>  : Message to be written/read-compare to/from "
          "the device memory\n";
